...
def data_load(args): 
	...
	dsets["source_tr"] = ImageList(tr_txt, transform=image_train(use_pasta=args.use_pasta, pasta_args=pasta_args))
...
def train_source(args):
	...
	while iter_num < max_iter:
		...
		if args.loss == "label_smooth_ce":
			...
			loss_str = "Focal"
			classifier_loss = FocalLoss(gamma=args.gamma)(outputs_source, labels_source)
		elif args.loss == "adaptive_focal_loss":
			loss_str = "AdaFocal"
			classifier_loss = FocalLossAdaptive(gamma=args.gamma)(outputs_source, labels_source)

		ent = Entropy(nn.Softmax(dim=1)(outputs_source)).mean()
		nll = nn.NLLLoss()(nn.LogSoftmax(dim=1)(outputs_source), labels_source)
		bsc = BrierScore()(outputs_source, labels_source)*100.0
		n_bins = 10
		ece = MulticlassCalibrationError(num_classes=args.class_num, n_bins=n_bins, norm="l1")(outputs_source, labels_source)*100.0
		mce = MulticlassCalibrationError(num_classes=args.class_num, n_bins=n_bins, norm="max")(outputs_source, labels_source)*100.0
   
		if iter_num % 50 == 0:
			log_str = 'Task: {}, Iter:{}; {} Loss = {:.2f} NLL = {:.4f} Ent = {:.4f} BSC = {:.4f} ECE = {:.4f} MCE = {:.4f}'.format(args.name_src, iter_num, loss_str, classifier_loss.item(), nll.item(), ent.item(), bsc.item(), ece, mce)
		...
		if iter_num % interval_iter == 0 or iter_num == max_iter:
			...
			if args.dset=='VISDA-C':
				acc_s_te, acc_list, req_metrics = cal_acc(dset_loaders['source_te'], netF, netB, netC, True)
				log_str = 'Task: {}, Iter:{}/{}; Accuracy = {:.2f}%'.format(args.name_src, iter_num, max_iter, acc_s_te) + '\n' + acc_list
				log_str += "\n Source: " + str(req_metrics)
				log_str += "\n"
				acc_t_te, acc_t_list, req_metrics = cal_acc(dset_loaders['test'], netF, netB, netC, True)
			...
			torch.save(netF.state_dict(), osp.join(args.output_dir_src, "source_F_ep_" + str(iter_num) + ".pt"))
			torch.save(netB.state_dict(), osp.join(args.output_dir_src, "source_B_ep_" + str(iter_num) + ".pt"))
			torch.save(netC.state_dict(), osp.join(args.output_dir_src, "source_C_ep_" + str(iter_num) + ".pt"))
